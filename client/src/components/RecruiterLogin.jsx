    ...
    return(
        <div className="absolute top-0 left-0 bottom-0 right-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center">
            <form  onSubmit={onSubmitHandler}className="relative bg-white p-10  rounded-xl  text-slate-500">
                <h1 className="text-center text-2xl text-neutral-700 font-medium capitalize">Recruiter {state}</h1>
                ...
                <button type='submit' className="bg-blue-600 w-full mt-1 text-white py-2 rounded-full">
                    { state === 'Login' ? 'login' : isTextSubmitted ?'create Account' : 'next'}
                </button>
                {
